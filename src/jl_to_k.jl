...
jl_to_kvec(iter, ::HasShape{0}, typ::Type{T}) where T = error("KdbConnect.jl error: can't handle arguments with HasShape{0}: itertype=$(typeof(iter)) type=$T")
...
jl_to_kvec(_, T::Union{IsInfinite,SizeUnknown}, _) = error("KdbConnect.jl error: Cannot convert an iterator with IteratorSize $T to K_Object")
